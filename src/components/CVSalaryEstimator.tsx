...
const CVSalaryEstimator = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        provinceDropdownRef.current instanceof HTMLElement &&
      ...
      if (
        districtDropdownRef.current instanceof HTMLElement &&
  ...
  const selectDistrict = (district: District) => {
    setDistrictCode(district.code.toString());
  ...
  return (
    <div className="relative max-w-4xl mx-auto">
      ...
      <div className="backdrop-blur-sm bg-white/30 p-8 rounded-2xl shadow-lg border border-white/20 relative z-10 ">
        <div className="absolute inset-0 bg-gradient-to-br from-purple-100/70 to-white/30 z-0"></div>

        <div className="relative z-10">
          <h1 className="text-3xl font-bold mb-8 text-center text-purple-800 drop-shadow-sm">
              ...
              <div className="backdrop-blur-sm bg-white/40 border border-white/60 rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300">
                ...
                <label htmlFor="cv-upload" className="block cursor-pointer">
                  <div className="flex flex-col items-center justify-center">
                    <svg
                      className="w-16 h-16 text-purple-600 opacity-80"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="relative " ref={provinceDropdownRef}>
                    <div 
                      className=" w-full p-3 backdrop-blur-sm bg-white/70 border border-white/60 text-purple-800 rounded-lg flex justify-between items-center cursor-pointer"
                      onClick={() => !isLoadingProvinces && setIsProvinceDropdownOpen(!isProvinceDropdownOpen)}
                    >
                      <span>{provinceName || "Chọn Tỉnh/Thành phố"}</span>
                      <svg 
                        className={`w-5 h-5 transition-transform duration-200 ${isProvinceDropdownOpen ? 'transform rotate-180' : ''}`} 
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24"
                      >
                    ...
                    {isProvinceDropdownOpen && (
                      <div className="absolute z-50 w-full mt-1 backdrop-blur-sm bg-white border border-purple-200/70 rounded-lg shadow-lg max-h-60 overflow-y-auto ">
                        <div className="sticky top-0 bg-white/90 backdrop-blur-sm p-2 border-b border-purple-100">
                          <div className="relative">
                            <input
                              type="text"
                              className="w-full p-2 pl-8 bg-white/70 border border-purple-200 rounded-md focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all"
                              placeholder="Tìm kiếm tỉnh/thành phố..."
                              value={provinceSearch}
                              onChange={(e) => setProvinceSearch(e.target.value)}
                            />
                            <svg
                              className="absolute left-2 top-2.5 w-5 h-5 text-purple-500"
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                            >
                        ...
                        {isLoadingProvinces ? (
                          ...
                          <div className="p-4 text-center text-purple-600 z-50">
                          ...
                          filteredProvinces.map((province) => (
                            <div
                              key={province.code}
                              className="p-2 hover:bg-purple-100/70 cursor-pointer transition-colors"
                ...
                <div>
                  ...
                  <div className="relative" ref={districtDropdownRef}>
                    <div 
                      className={`w-full p-3 backdrop-blur-sm bg-white/70 border border-white/60 text-purple-800 rounded-lg flex justify-between items-center ${!provinceCode || isLoadingDistricts ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                      onClick={() => provinceCode && !isLoadingDistricts && setIsDistrictDropdownOpen(!isDistrictDropdownOpen)}
                    >
                      <span>
                        {districtName 
                          ? districtName 
                      ...
                      <svg 
                        className={`w-5 h-5 transition-transform duration-200 ${isDistrictDropdownOpen ? 'transform rotate-180' : ''}`} 
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24"
                      >
                    ...
                    {isDistrictDropdownOpen && provinceCode && (
                      <div className="absolute z-50 w-full mt-1 backdrop-blur-sm bg-white border border-purple-200/70 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                        <div className="sticky top-0 bg-white backdrop-blur-sm p-2 border-b border-purple-100">
                          <div className="relative">
                            <input
                              type="text"
                              className="w-full p-2 pl-8 bg-white/70 border border-purple-200 rounded-md focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all"
              ...
              {isUploading && (
                <div className="mt-4">
                  ...
                  <p className="text-sm text-purple-600 mt-2 text-center">
                    Đang phân tích CV của bạn... {uploadProgress}%
                  </p>
                </div>
              )}
            </form>
                ...
                <div className="mt-8 border-t border-purple-200/50 pt-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <div className="backdrop-blur-md bg-white/30 p-4 rounded-lg border border-white/60 shadow-sm group hover:shadow-md transition-all">
                      ...
                      <p className="text-lg text-purple-800 font-semibold group-hover:translate-x-1 transition-transform">
                        {salaryResponse.positionLevel}
                      </p>
                    </div>
                    <div className="backdrop-blur-md bg-white/30 p-4 rounded-lg border border-white/60 shadow-sm group hover:shadow-md transition-all">
                      <h3 className="text-sm font-medium text-purple-600 mb-1">
                  ...
                  {salaryResponse.improvementSuggestions && (
                    <div className="mt-6 backdrop-blur-md bg-white/30 p-5 rounded-lg border border-white/60 shadow-md">
